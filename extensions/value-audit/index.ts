...
import { appendNote, exportReport, listText, readJournalTail, todoText } from "./subcommands";
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("value-audit", {
    description:
      "价值审计：这个项目凭什么收钱（子命令：where 路径 / open 打开档案 / todo 待办 / list 全部项目 / note 记录决策 / export 导出 / focus 聚焦审计 / stats 匿名统计开关）",
    handler: async (args, ctx) => {
      ...
      if (sub !== "" && head !== "focus") {
        say(`未知子命令：${head}（支持 where/open/todo/list/note/export/focus）`);
        return;
      }
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      const written = store.writeReport(applied.markdown, slug);
      ...
      if (telemetryConfigured() && ctx?.hasUI && getConsent() === "unset") {
        const ok = await ctx.ui.confirm(
          "匿名使用统计",
          "是否开启匿名统计帮助改进工具？只上报版本/审计次数/灯色/操作系统，不含任何代码与内容，可随时 /value-audit stats off 关闭",
        );
        setConsent(ok);
      }
