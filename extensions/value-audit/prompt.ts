...
export type IntentKind = "unfilled" | "noncommercial" | "commercial" | "fundraising";
...
/**
 * v0.9 意图前置：确定性解析 facts 第 0 节"项目当前意图"。
 * 含商业关键词即按商业化审计（工具前提）；只写自用/开源或否定商业化才降级；未填默认商业化但报告必须写出假设。
 */
export function readIntent(factsText: string): Intent {
  const line = factsText.split("\n").find((l) => l.startsWith("- 项目当前意图"));
  const raw = line ? line.slice(line.indexOf("：") + 1).replace(/（示例[：:][^）]*）/g, "").trim() : "";
  if (!raw || raw.includes("[待补充]")) return { kind: "unfilled", raw: "" };
  const negRe = /(不|没有|无|暂无|暂不|非)(打算|准备|想|会|考虑|计划)?(商业化|变现|收费|付费|卖|融资|授权|盈利|赚钱)/g;
  const hasNeg = negRe.test(raw);
  negRe.lastIndex = 0; // 带 g 标志的 test 会推进 lastIndex，复位后再 replace
  const rest = raw.replace(negRe, "");
  if (/融资|路演|投资人/.test(rest)) return { kind: "fundraising", raw };
  if (/商业化|变现|收费|付费|卖|授权|订阅|客户|收入|盈利|赚钱/.test(rest)) return { kind: "commercial", raw };
  if (hasNeg || /自用|内部|个人|开源|分享|玩具|练手|学习/.test(rest)) return { kind: "noncommercial", raw };
  return { kind: "commercial", raw }; // 填了但无法归类：按工具前提审计，报告会写出假设供用户纠正
}

/** 审计假设节：把意图翻译成报告第 0/3/4/5/7 节的硬规则，避免 LLM 自行解读 */
function intentSection(intent: Intent): string {
  const head = `## 审计假设（意图前置：工具确定性解析自 facts 第 0 节"项目当前意图"；整体红灯时红灯变体优先于本节的第 3-5 节规则）\n\n`;
  const bleed = `- 第 4 节：正在发生的不可逆损失（数据被硬删/密钥无备份等）的条目标 [止血]；第 7 节最小下一步存在 [止血] 条目时必须先选它（处方纪律 6）。`;
  switch (intent.kind) {
    case "unfilled":
      return `${head}- facts 第 0 节未填写 → 本次按默认假设"打算商业化"审计（现行结构）。
- 报告第 0 节结论之后固定一行：「审计假设：facts 第 0 节未填，本报告默认按"打算商业化"审计；若实为自用/内部工具，只需看第 1 节核心逻辑与第 4 节标 [止血] 的条目，其余可略；在 facts 第 0 节填一行意图后重跑，P0 会按意图调整」。
- 第 4 节：商业验证类动作（访谈/定价/渠道/付费意向）可标 P0，但"方向"栏必须注明「前提：意图=商业化（facts 第 0 节未填）」。
${bleed}`;
    case "noncommercial":
      return `${head}- facts 第 0 节 = 「${intent.raw}」→ 判定为自用/开源分享，不按商业化前提开方。
- 报告第 0 节结论之后固定一行：「审计假设：意图=自用/开源分享（facts 第 0 节）；本报告只回答"若有一天想商业化，最先要补什么"，不催你去卖」。
- 第 2 节七问照常回答（它们回答"有没有潜在商业价值"，与意图无关）；第 3 节压缩为一小节「若未来商业化，最先要补的 3 件事」（省略五环表与观测接入建议，观测只保留画像分级一行，不强制观测 backlog）。
- 第 4 节：P0 只允许两类——[止血] 与「决定是否商业化」这一判断题（零成本）；商业验证类动作（访谈/定价/渠道/付费意向）一律 P1 并注明「前提：决定商业化」。
- 第 5 节省略为一句话（不出问答稿，questions 仍提交 32 题标记）。
${bleed}`;
    case "fundraising":
      return `${head}- facts 第 0 节 = 「${intent.raw}」→ 正在融资：按商业化前提审计，且第 5 节投资人问答稿输出完整 32 题（[待验证] 的题只给回答框架 + 验证动作，不编数字）。
- 报告第 0 节结论之后固定一行：「审计假设：意图=${intent.raw}（facts 第 0 节）」。
${bleed}`;
    default:
      return `${head}- facts 第 0 节 = 「${intent.raw}」→ 按商业化前提审计（现行结构）；第 5 节精简版。
- 报告第 0 节结论之后固定一行：「审计假设：意图=${intent.raw}（facts 第 0 节）」。
${bleed}`;
  }
}
...
2. **诚实**：每个结论标注来源，四类标记：
   - [代码证实]：必须附文件路径，且每个路径写完整相对路径（同目录多文件也逐个写全，禁止
     "internal/model/task.go、goal.go" 这类省略目录写法）；路径后不要紧跟括号说明（"a.go（函数名）"会被当成路径），
     说明文字写在方括号外；不得用"同上"代替路径——保存时工具会逐个核实路径存在性，没附路径的标记会被标 ⚠️缺路径
...
3. **证据优先**：画像是入口不是全部。审计中你应该用 read 工具查看关键文件核实证据（只读，
   ...
   画像若列出「本地数据文件」，可用 \`sqlite3 -readonly\`（或等价只读方式）做**聚合**查询辅助用户画像与用量判断：
   只取计数/分布/量级，不 SELECT 明细行，不引用任何可识别信息，不复制不导出文件；facts 含"不要读取本地数据文件"则跳过
   （隐私红线见知识库 value-judgment 附加检查 4）。嵌套仓库（画像已折叠）是第三方代码，不作为本项目能力或商业化证据。
...
5. **完成后必须调用工具 value_audit_save 提交结果**（只调用一次），不要把报告全文直接回复给用户。`);
  ...
  sections.push(intentSection(readIntent(o.factsText)));
...
## 0. 审计结论 —— 🔴/🟡/🟢 + 一句话理由（引用触发条款）+ 一行「审计假设」（原句见上方"审计假设"节）
...
## 4. 行动 backlog（验证动作优先于代码改动，遵守知识库"处方纪律"；每条：ID、标题、
   ...
   P0 优先给零代码验证动作；止血类条目标 [止血]；P0 必须与"审计假设"节一致（处方纪律 10）；
   若存在上次遗留未动的 P0 条目，本节开头先列出提醒）
## 5. 投资人问答稿（**默认精简版**：仅回答已有 [代码证实]/[用户提供]/[联网检索] 证据的题目 + 缺口清单 +
   一段话说明何时值得做完整版；仅当上方"审计假设"节判定为正在融资时才 32 题全出；意图为自用/开源分享时省略为一句话。
...
## 7. 档案资产与下一步（峰终收尾：报告正文永远以本节结束，不以风险/缺口收尾）
   ...
   - 最小下一步：从 backlog 挑 1 条 P0（单动作、当天可完成；有 [止血] 条目必须先选它）；红灯项目挑转向建议中的最小验证动作
