...
import { OBS_PATH_WORDS, collectObservability } from "./observability";
  ...
  const walkDir = (dir: string, rel: string, depth: number): void => {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        if (obsPaths.length < 10 && OBS_PATH_WORDS.some((w) => lower.includes(w))) obsPaths.push(relPath);
      }
    }
  };
  walkDir(root, "", 0);
  if (treeLines.length >= MAX_TREE) truncated = true;
  return { treeLines, fileCount, signalPaths, obsPaths, testCount, truncated };
}
...
/** 提交目录热度：开发投入用脚投票，审计时对照官方定位发现漂移 */
function commitHeat(root: string): string[] {
...
export function collectProfile(root: string): ProjectProfile {
  ...
  const obs = collectObservability(root, w.obsPaths);
  ...
  const text = [
    ...
    `## git`,
    ...g.lines,
    ...(g.commit ? [`- 近 200 次提交目录热度（开发重心用脚投票，审计时对照定位）: ${commitHeat(root).join("，") || "无"}`] : []),
    ``,
