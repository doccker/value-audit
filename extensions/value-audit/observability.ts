...
function detectEventCalls(root: string): string[] | null {
  if (gitRun(root, ["rev-parse", "--is-inside-work-tree"]) === null) return null;
  const out = gitRun(root, ["grep", "-l", "-E", EVENT_CALL_PATTERN, "--", ":!*.lock", ":!*.min.js", ":!node_modules", ":!dist"]);
 ...
 */
export function collectObservability(root: string, obsPaths: string[]): ObservabilityResult {
  const deps = detectDeps(root);
  const scripts = detectScripts(root);
  const calls = detectEventCalls(root);
